...
class LineChart {
  ...
  render() {
    this.cleanData();
    this.salesPerYear();
    let salesValues = Object.values(this.salesByYear);
    this.mean = calculateMean(salesValues);
    this.median = calculateMedian(salesValues);
    console.log("Line chart Mean:", round(this.mean), "Median:", round(this.median));
    this.drawAxis();
    this.drawLines();
    this.drawLabels();
  }
  ...
  drawLines() {
    ...
    stroke("#ff0000");
    strokeWeight(2);
    let meanHeight = map(this.mean, 0, this.maxSales, 0, this.chartHeight);
    line(0, -meanHeight, this.chartWidth, -meanHeight);

    stroke("#00ff00");
    let medianHeight = map(this.median, 0, this.maxSales, 0, this.chartHeight);
